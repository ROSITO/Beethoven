...
Validation is appended to the score as a governed `validate` task, so it appears
in the trace, desktop progression, event stream, and session artifacts. Known
read-only checks run in `ask` and `read-only` modes. Unknown or mutating
